...
const AIResumeAnalyzer = () => {
  ...
  const handleAnalyze = async () => {
    try {
      ...
      const uploadResponse = await API.post("/ai/upload-resume", formData);
      console.log("Upload Response:", uploadResponse.data);

      const resumeText = uploadResponse.data.text;

      // STEP 2 → Analyze Resume
      const analysisResponse = await API.post("/ai/analyze-resume", {
        resumeText,
        jobDescription,
      });
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex justify-center items-center p-6">
      <div className="bg-white shadow-xl rounded-2xl p-8 w-full max-w-3xl">
        ...
        >
          {loading ? (
            <LoadingState compact label="AI Analyzing..." />
          ) : (
            "Analyze Resume"
          )}
        </button>
        {analysis && (
          <div className="mt-10 space-y-6">
            ...
            <div className="bg-linear-to-r from-black to-gray-800 text-white p-8 rounded-3xl shadow-xl">
              <h2 className="text-2xl font-bold mb-5">AI Match Score</h2>
